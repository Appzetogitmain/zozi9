...
const DeliveryLegalPage = ({ type }) => {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            setIsLoading(true);
            try {
                const res = await deliveryApi.getLegalPage('DELIVERY', type);
                if (res.data?.success) {
                    setContent(res.data.result?.content || '');
                }
            } catch (err) {
                console.error(`Failed to fetch delivery ${type}:`, err);
            } finally {
                setIsLoading(false);
            }
        };
                ...
                <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
                    <div className="flex items-center gap-4 mb-8 pb-6 border-b border-slate-100">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500 font-medium mt-1">For Delivery Partners</p>
                        </div>
                    </div>
